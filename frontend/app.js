const API = '/api';
let entries = [], filtered = [], idx = 0, dataset = 'welllog';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Dataset change
    $('dataset-select').onchange = e => { dataset = e.target.value; refresh(); };

    // Actions
    $('btn-download').onclick = () => runAction('/api/download', `Download ${dataset}?`);
    ...
    $('btn-ocr').onclick = () => {
        ...
        }).then(r => r.json()).then(d => { alert(d.message); refreshLogs(); });
    };
    ...
    document.onkeydown = e => { if (e.key === 'ArrowLeft') nav(-1); if (e.key === 'ArrowRight') nav(1); };
...
async function loadDatasets() {
    try {
        ...
        $('dataset-select').innerHTML = data.datasets.map(d =>
            `<option value="${d}" ${d === data.default ? 'selected' : ''}>${d}</option>`
        ).join('');
...
async function refresh() {
    try {
        const data = await (await fetch(`${API}/entries?dataset=${dataset}`)).json();
        ...
        $('status-dot').className = `w-2 h-2 rounded-full ${st.running ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`;
    } catch (e) { console.error(e); }
}
...
function updateStats() {
    ...
    $('stat-rate').textContent = total ? ((ok / total) * 100).toFixed(0) + '%' : '0%';
    $('stat-errors').textContent = total - ok;

    const recent = entries.slice(-10).reverse();
    $('jobs-table').innerHTML = recent.map(e => `
        <tr class="border-b">
            <td class="py-2 font-mono">${e.filename}</td>
            <td class="py-2"><span class="px-2 py-0.5 rounded-full text-xs ${e.status === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">${e.status}</span></td>
            <td class="py-2">${e.model || '-'}</td>
            <td class="py-2"><button class="text-blue-600 hover:underline" onclick="view('${e.filename}')">View</button></td>
        </tr>
    `).join('') || '<tr><td colspan="4" class="py-4 text-center text-gray-400">No data</td></tr>';
}
...
function filter() {
    ...
    if (filtered.length) show(idx); else clear();
}
...
function show(i) {
    ...
    $('image-panel').innerHTML = e.status === 'success'
        ? `<img src="/images/${e.filename}?dataset=${dataset}" class="max-w-full">`
        : '<div class="text-red-500">Error</div>';
    $('ocr-panel').innerHTML = e.status === 'success'
        ? `<div class="text-left whitespace-pre-wrap text-sm">${e.answer || ''}</div>`
