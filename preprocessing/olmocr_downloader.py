...
def download_olmocr(
    ...
    for subset in SUBSETS:
        ...
        ds = load_dataset(
            "allenai/olmOCR-mix-1025",  # nosec: B615 - loading training data, not code
            subset,
            split=split,
            streaming=True,
        )
        ...
        for row in ds:
            ...
            url_hash = hashlib.md5(row["url"].encode(), usedforsecurity=False).hexdigest()[:8]
            ...
            try:
                ...
                total += 1
                print(
                    f"  [{existing_count + count}/{limit_per_subset}] {filename} ({word_count} words)"
                )
